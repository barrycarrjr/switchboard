...
4. **Manual Control:** While the Quota Watcher is set to "Switch automatically", a tool that has lanes belongs to its lane order: an account picked by hand would be switched back at the Watcher's next check, within five minutes. So for that tool the Accounts page and the tray menu show which account is in use but do not offer to switch it, and the card's "Set by lane order" link opens the Lanes tab. Reorder the lanes to change which account new sessions use. To choose by hand instead, set "When an account runs out" to "Tell me" or "Do nothing" in the tray menu; on "Tell me" the Watcher suggests the lane order's pick once, not every five minutes. `switchboard use` still switches from a terminal, because it cannot tell whether a Watcher is running, and it says what a running one would do.
